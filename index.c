    ...
    if (step == 0) { // step 0: read sequences
		...
		if (s->seq) {
			...
			if (old_m != m){
                ...
                // 采用新的对齐内存分配函数
                // 计算旧内存大小和新内存大小
				size_t old_size_bytes = old_m * sizeof(mm_idx_seq_t);
				size_t new_size_bytes = m * sizeof(mm_idx_seq_t);
				// 调用新的对齐 realloc 函数，并指定8字节对齐
				p->mi->seq = (mm_idx_seq_t*)realloc_aligned(p->mi->seq, old_size_bytes, new_size_bytes, 8);
...
{
    fprintf(stderr, "mm_idx_gen 00\n");
	pipeline_t pl;
	if (fp == 0 || mm_bseq_eof(fp)) return 0;
	memset(&pl, 0, sizeof(pipeline_t));
    fprintf(stderr, "mm_idx_gen 01 after pipeline_t\n");

	pl.mini_batch_size = (uint64_t)mini_batch_size < batch_size? mini_batch_size : batch_size;
	pl.batch_size = batch_size;
	pl.fp = fp;
	pl.mi = mm_idx_init(w, k, b, flag);
    fprintf(stderr, "mm_idx_gen 02 after pl.mi\n");

	kt_pipeline(n_threads < 3? n_threads : 3, worker_pipeline, &pl, 3);
    fprintf(stderr, "mm_idx_gen 03 after kt_pipeline\n");

	if (mm_verbose >= 3)
		fprintf(stderr, "[M::%s::%.3f*%.2f] collected minimizers\n", __func__, realtime() - mm_realtime0, cputime() / (realtime() - mm_realtime0));

	mm_idx_post(pl.mi, n_threads);
    fprintf(stderr, "mm_idx_gen 04 after mm_idx_post\n");
	if (mm_verbose >= 3)
